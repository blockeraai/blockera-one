...
| `templates-nav-config.ts` | Static purpose-nav IA |
| `templates-matchers.ts` | Slug / custom / author matchers |
| `use-templates-data.ts` | Entity fetch, counts, dynamic CPT/author rows |
...
- **Templates:** purpose sections; Header opens parts sub-screen; Back from parts returns to Templates menu; Back from Templates returns to Design root; clicking a category with a base template opens canvas preview.
